...
Shader "WaterAmplifyShader"
{
	Properties
	{
		...
		_GradientNoiseScale("Gradient Noise Scale", Float) = 0
		_GradientNoiseSpeed("Gradient Noise Speed", Float) = 0
		[ASEEnd]_StepEdge("StepEdge", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput ) : SV_Target
			{
				...
				float simplePerlin2D63 = snoise( ( texCoord61 + ( ( _TimeParameters.x ) * ( _GradientNoiseSpeed * 0.0 ) ) )*_GradientNoiseScale );
				...
				float mulTime53 = _TimeParameters.x * _WaveSpeed;
				float3 temp_cast_0 = (step( _StepEdge , ( simplePerlin2D63 + ( tex2DNode55.r * sin( ( ( tex2DNode55.r * _Frequency ) + mulTime53 ) ) ) ) )).xxx;
		...
		Pass
		{
			...
			float4 Frag( VertexOutput packedInput  ) : SV_Target
			{
				...
				SurfaceDescription surfaceDescription = (SurfaceDescription)0;
				float2 texCoord61 = packedInput.ase_texcoord.xy * float2( 1,1 ) + float2( 0,0 );
				float simplePerlin2D63 = snoise( ( texCoord61 + ( ( _TimeParameters.x ) * ( _GradientNoiseSpeed * 0.0 ) ) )*_GradientNoiseScale );
				simplePerlin2D63 = simplePerlin2D63*0.5 + 0.5;
...
ASEEND*/
//CHKSM=D4963E0FD0CCBEEF9B60395D279FE942AA2B04DB